...
const mark = async (id) => {
  try {
    ...
    console.error("Error Marking", error.message);
  }
};
...
const sendAllFiles = async (id, name, number, agent) => {
...
const fetchAndSendMessage = async () => {
  try {
    const query = `SELECT * FROM follow_up_cust WHERE sent_status = 'not-sent' ORDER BY id DESC LIMIT 1`;
    const rows = await poolConnection.query(query);

    if (rows.length > 0) {
      const { id, contact, name, agent } = rows[0];
      await sendAllFiles(id, name, contact, agent);
      console.log("Message sent successfully");
    } else {
